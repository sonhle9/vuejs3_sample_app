<template>
  ...
  <h2>Computed Total - {{ total }}</h2> <!-- lưu cache sử dụng lại -->
  ...
  <button @click="changeFullName">Change fullname</button> <!-- Video 26 set and get -->

  <!-- Watchers  video 27  can using after call API -->
  <h2>Volume Tracker (0-100)</h2>
  ...
  watch: {
    volume: {
      handler(newValue, oldValue) {
        if (newValue > oldValue && newValue === 90) {
          alert(
